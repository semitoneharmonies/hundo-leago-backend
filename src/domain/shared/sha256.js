...
function requireBytes(value, functionName) {
  if (!(value instanceof Uint8Array)) {
    throw new TypeError(
      `${functionName} requires a Uint8Array`
    );
  }
  return value;
}
...
function digestBytes(input) {
  ...
  const byteLength =
    Math.ceil((input.length + 9) / 64) * 64;
...
function sha256Bytes(value) {
  return digestBytes(
    requireBytes(value, "sha256Bytes")
  );
}
...
function sha256HexFromBytes(value) {
  return bytesToHex(
    digestBytes(
      requireBytes(value, "sha256HexFromBytes")
    )
  );
}
...
function sha256Hex(value) {
  if (typeof value !== "string") {
    throw new TypeError("sha256Hex requires a string");
  }
