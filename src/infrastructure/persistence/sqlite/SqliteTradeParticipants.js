...
function createSqliteTradeParticipants(database) {
  ...
  function list(input) {
    if (!rows) {
      ...
      if (database.pragma('user_version', { simple: true }) < 64) return [];
  ...
  function create(command) {
    ...
    if (database.pragma('user_version', { simple: true }) < 64) throw new TradeLifecyclePolicyError(TRADE_LIFECYCLE_CODES.stateInvalid);
