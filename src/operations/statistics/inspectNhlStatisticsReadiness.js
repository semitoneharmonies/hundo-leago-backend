...
function inspectNhlStatisticsReadiness({ database, nhlSeasonKey, minimumPlayerCount = 200 } = {}) {
  ...
  const missingRosterIdentities = database.prepare(`
    ...
    ORDER BY player.id
  `).all(season);
  const conflictingLocks = database.prepare(`
  ...
  if (conflictingLocks.length) issues.push("An unfinished matchup uses another provider. Do not switch its source or replace its baseline.");
