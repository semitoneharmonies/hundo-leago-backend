...
async function runThreeTeamTradeFlow(t, { createRuntime, IDS, NOW_MS, authenticated, sourceState }) {
  ...
  await scenario('the additive migration preserves all pre-existing records', async () => {
    ...
    db.prepare("UPDATE application_metadata SET metadata_value = '63' WHERE metadata_key = 'data_model_version'").run();
    db.pragma('user_version = 63');
    const historicalParticipants = require('../../src/infrastructure/persistence/sqlite/SqliteTradeParticipants').createSqliteTradeParticipants(db);
    const historicalBytes = db.serialize();
    assert.deepEqual(historicalParticipants.list({ leagueId: IDS.league, tradeId: uuid(999999) }), []);
    assert.throws(() => historicalParticipants.create({ participantTeamIds: [IDS.teamA, IDS.teamB, teamC] }));
    assert.equal(historicalBytes.equals(db.serialize()), true);
    db.pragma('user_version = 64');
    assert.throws(() => historicalParticipants.list({ leagueId: IDS.league, tradeId: uuid(999999) }), /no such table/);
    db.pragma('user_version = 63');
    ...
    db.exec(fs.readFileSync(path.join(__dirname, '../../database/migrations/0064_add_three_team_trade_participants.sql'), 'utf8'));
    db.pragma('user_version = 64');
    ...
    assert.equal(db.prepare("SELECT metadata_value FROM application_metadata WHERE metadata_key = 'data_model_version'").get().metadata_value, '64');
  });
