...
for (const rollback of [false, true]) test('buyout clears only its saved display order and preserves it on rollback: ' + rollback, (t) => {
  const { database, repository } = createRuntime(t, rollback ? { candidateCardSummerSynchronizer: { synchronize() { throw new Error('injected after ownership deletion'); } } } : {});
  database.prepare('INSERT INTO roster_display_order_sets (id,league_id,season_id,team_id,updated_by_user_id,created_at_ms,updated_at_ms,version) VALUES (?,?,?,?,?,?,?,1)').run(uuid(900),IDS.league,IDS.season1,IDS.team1,IDS.user,NOW_MS,NOW_MS);
  database.prepare('INSERT INTO roster_display_order_entries (id,league_id,order_set_id,ownership_id,position_group,display_order,created_at_ms) VALUES (?,?,?,?,?,?,?)').run(uuid(901),IDS.league,uuid(900),IDS.ownership,'F',1,NOW_MS);
  const before=database.serialize(), orderSet=database.prepare('SELECT * FROM roster_display_order_sets').all();
  if(rollback){assert.throws(()=>repository.buyOut(command()));assert.deepEqual(database.serialize(),before);}
  else {repository.buyOut(command());assert.equal(count(database,'roster_display_order_entries'),0);assert.deepEqual(database.prepare('SELECT * FROM roster_display_order_sets').all(),orderSet);assert.deepEqual(database.pragma('foreign_key_check'),[]);const after=database.serialize();repository.buyOut(command());assert.deepEqual(database.serialize(),after);}
});
