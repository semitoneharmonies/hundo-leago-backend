...
-- Approved 2026-09-22: highest AAV, then longest term. Preserve historical rows and decision codes.

-- New decisions use AAV/term checks; historical total-first decisions retain their recorded validation.
...
CREATE TABLE free_agent_draft_player_allocations_ranking_rebuild (
  id TEXT PRIMARY KEY
...
INSERT INTO free_agent_draft_player_allocations_ranking_rebuild SELECT * FROM free_agent_draft_player_allocations;

DROP TABLE free_agent_draft_player_allocations;

ALTER TABLE free_agent_draft_player_allocations_ranking_rebuild RENAME TO free_agent_draft_player_allocations;
    ...
    (
      ...
      AND EXISTS (
        ...
        WHERE free_agent_draft_auction_participants.league_id =
          ...
          AND (
            (( (auction_bids.total_value_cents / auction_bids.term_years) + CASE WHEN (auction_bids.total_value_cents % auction_bids.term_years) * 2 >= auction_bids.term_years THEN 1 ELSE 0 END )) > free_agent_draft_auction_participants.minimum_aav_cents
            OR ((( (auction_bids.total_value_cents / auction_bids.term_years) + CASE WHEN (auction_bids.total_value_cents % auction_bids.term_years) * 2 >= auction_bids.term_years THEN 1 ELSE 0 END )) = free_agent_draft_auction_participants.minimum_aav_cents AND auction_bids.term_years > free_agent_draft_auction_participants.minimum_term_years)
          )
      )
    )
    OR (
      ...
      AND (
        ...
        OR EXISTS (
          ...
          WHERE free_agent_draft_player_allocations.league_id =
            ...
            AND (
              (( (auction_bids.total_value_cents / auction_bids.term_years) + CASE WHEN (auction_bids.total_value_cents % auction_bids.term_years) * 2 >= auction_bids.term_years THEN 1 ELSE 0 END )) > free_agent_draft_player_allocations.restricted_minimum_aav_cents
            OR ((( (auction_bids.total_value_cents / auction_bids.term_years) + CASE WHEN (auction_bids.total_value_cents % auction_bids.term_years) * 2 >= auction_bids.term_years THEN 1 ELSE 0 END )) = free_agent_draft_player_allocations.restricted_minimum_aav_cents AND auction_bids.term_years >= free_agent_draft_player_allocations.restricted_minimum_term_years)
            )
        )
      )
    )
      ...
      AND (
        ...
        OR EXISTS (
          ...
          WHERE free_agent_draft_auction_participants.league_id =
            ...
            AND (
              (( (NEW.total_value_cents / NEW.term_years) + CASE WHEN (NEW.total_value_cents % NEW.term_years) * 2 >= NEW.term_years THEN 1 ELSE 0 END )) > free_agent_draft_auction_participants.minimum_aav_cents
            OR ((( (NEW.total_value_cents / NEW.term_years) + CASE WHEN (NEW.total_value_cents % NEW.term_years) * 2 >= NEW.term_years THEN 1 ELSE 0 END )) = free_agent_draft_auction_participants.minimum_aav_cents AND NEW.term_years > free_agent_draft_auction_participants.minimum_term_years)
            )
        )
      )
    )
    ...
    AND EXISTS (
      ...
      WHERE auction_contexts.league_id = NEW.league_id
        ...
        AND auction_contexts.fad_origin =
          'restricted_no_improvement_fallback'
        AND NOT (
          (( (NEW.total_value_cents / NEW.term_years) + CASE WHEN (NEW.total_value_cents % NEW.term_years) * 2 >= NEW.term_years THEN 1 ELSE 0 END )) > free_agent_draft_player_allocations.restricted_minimum_aav_cents
            OR ((( (NEW.total_value_cents / NEW.term_years) + CASE WHEN (NEW.total_value_cents % NEW.term_years) * 2 >= NEW.term_years THEN 1 ELSE 0 END )) = free_agent_draft_player_allocations.restricted_minimum_aav_cents AND NEW.term_years >= free_agent_draft_player_allocations.restricted_minimum_term_years)
        )
    )
  THEN RAISE(
    ABORT,
      ...
      (
        ...
        AND EXISTS (
          ...
          WHERE auction_bids.league_id = NEW.league_id
            ...
            AND auction_bids.status = 'active'
            AND (
              (( (auction_bids.total_value_cents / auction_bids.term_years) + CASE WHEN (auction_bids.total_value_cents % auction_bids.term_years) * 2 >= auction_bids.term_years THEN 1 ELSE 0 END )) > NEW.minimum_aav_cents
            OR ((( (auction_bids.total_value_cents / auction_bids.term_years) + CASE WHEN (auction_bids.total_value_cents % auction_bids.term_years) * 2 >= auction_bids.term_years THEN 1 ELSE 0 END )) = NEW.minimum_aav_cents AND auction_bids.term_years > NEW.minimum_term_years)
            )
