...
const JOB_NAME =
  "free-agent-drafts:fallback-activation:target";
const JOB_TYPE =
  FREE_AGENT_DRAFT_JOB_TYPE_BY_OCCURRENCE
    .fallback_activate;
...
function requireMethod(
  ...
  if (!value || typeof value[method] !== "function") {
    throw new TypeError(
      `FAD fallback-activation runner requires ${description}`
    );
  }
}
...
function requireActivationDescriptor(value) {
  ...
  if (
    ...
    binding.type !== "fallback_activate" ||
    ...
    (
      binding.recoveryId !== null &&
      !UUID_PATTERN.test(binding.recoveryId || "")
    )
  ) {
    throw new TypeError(
      "FAD fallback-activation runner received a noncanonical activation descriptor"
    );
  }
  return value;
}
...
function requireClaimed(
  ...
  if (
    ...
    throw new TypeError(
      "FAD fallback-activation runner received an invalid acquired claim"
    );
  }
  ...
  if (
    ...
    throw new TypeError(
      "FAD fallback-activation runner received a mismatched acquired claim"
    );
  }
  return claimed;
}
...
function requireTerminal(result, claimed) {
  ...
  if (
    ...
    !UUID_PATTERN.test(result.sourceAuctionId || "") ||
    result.sourceAuctionId === result.auctionId ||
...
function createActivateFreeAgentDraftFallbackAuctionsJob({
  repository,
  activationService,
  clock,
  secureRandom,
  ...
  if (
    ...
    throw new TypeError(
      "FAD fallback-activation runner configuration is invalid"
    );
  }
  ...
  return createJobRunner({
    ...
    async execute() {
      ...
      if (!Array.isArray(due)) {
        throw new TypeError(
          "FAD fallback-activation runner requires a due-occurrence array"
        );
      }
      ...
      for (const occurrence of activationDue) {
        try {
          ...
          if (
            ...
            throw new TypeError(
              "FAD fallback-activation runner received work before its schedule"
            );
          }
